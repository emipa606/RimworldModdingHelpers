...
try:
    ...
    twoFactor = subprocess.run(['steamguard', '--verbosity', 'error'], capture_output=True, text=True).stdout.strip()
    result = user.login(username=username, password=password, twofactor_code=twoFactor)
...
try:
    ...
    while user.session.verify:
        ...
        if notificationsDiv:
            ...
            archiveMode = True
            sendtestpost("Comment monitor", "No new notifications, using the last 5 for testing")
            unreadNotifications = soup.findAll(
        ...
        for notification in unreadNotifications:
            ...
            for comment in allComments:
                hiddenContent = comment.find("div", {"class": "comment_hidden_content"})
                if hiddenContent:
                    continue
                ...
                senddiscordpost(link, modName, author, authorPage, imageUrl, TEXT)
                sleep(5)
